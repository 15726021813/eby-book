...
//		System.out.println("sss");
		LoadLib("C:\\Users\\Dell\\Desktop\\RfidTest",rfid_def.VER_WINDOWS,rfid_def.AR_X86);
		char[] buffer = new char[32];
		System.out.println("版本号："+RDR_GetLibVersion(buffer,32));
		int  i = COMPort_GetEnumItem(1,buffer,32);
		//打开连接
		String connStr = "RDType=RD201;CommType=COM;COMName=COM5;BaudRate=38400;Frame=8E1;BusAddr=255";
		Long hrOut = 1L;
		int res = RDR_Open(connStr,hrOut);
		System.out.println("设备句柄："+res+":"+hrOut);

		Long datares = RDR_CreateInvenParamSpecList();
		System.out.println("空中接口协议参数列表数据节点:"+datares);

		//i1 = 0 成功，非0不成功
		//以 iso15693扫描，也就是以下通过iso15693协议的都能扫描到
		rfidlib_AIP_ISO15693.LoadLib("C:\\Users\\Dell\\Desktop\\RfidTest",rfid_def.VER_WINDOWS,rfid_def.AR_X86);
		Long l = rfidlib_AIP_ISO15693.ISO15693_CreateInvenParam(datares,Utils.hexToByte("0"),Utils.hexToByte("0"),Utils.hexToByte("0"),Utils.hexToByte("0"));

		int i1 = RDR_TagInventory(hrOut, Utils.hexToByte("1"),Utils.hexToByte("0"),null,datares);
		System.out.println("寻找标签就绪："+i1);
		int tabData = RDR_GetTagDataReportCount(hrOut);
		System.out.println("寻找标签的数量:"+tabData);

		long tabdata = RDR_GetTagDataReport(hrOut,rfid_def.RFID_SEEK_FIRST);
		//12217392  11041272
		System.out.println("获取其中一个的数据节点连接："+tabdata);
		while (tabdata != 0){
			...
			System.out.println(Utils.bytesToHexString(uid));

			tabdata =  RDR_GetTagDataReport(hrOut,rfid_def.RFID_SEEK_NEXT);
		}
